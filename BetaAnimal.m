classdef BetaAnimal < handle %Now with the new movement system that checks for bounds
    properties
        ...
        % [7]: Awareness Modifier (higher gene = better at see threats/food
        GENECOUNT = 7; % A constant value of genes used for initialization.
        saturation; %2-element vector for hunger and thirst respectively
    ...
    methods
        function obj = BetaAnimal(~, envsize) %Constructor
           rng('shuffle');
           ...
           obj.env_size = envsize; %Internalize the environment size to decrease function usages
           obj.state_machine = "Roaming"; %Used for checking conditions, but not much else
           
           uvec = [(2*rand)-1, (2*rand)-1]; %Vector for direction
           obj.move_dir = uvec/norm(uvec); %Take the unit vector of the direction vector
           obj.move_speed = 3 + (2*obj.genes(2)); %Speed is partially determined by genetics
           obj.move_duration = randi([5, 10]); %Set movement duration to be semi-random
           
           obj.lifespan = round(500 + (500 * obj.genes(1))); %Lifespan is determined by genetics
           obj.initial_lifespan = obj.lifespan; %Copy over the lifespan variable
           obj.isDead = 0; %A boolean value that indicates if an animal is dead
           obj.awareness = 10 + 15*obj.genes(7); %awareness radius of animal
           
           obj.saturation = 90+round(20*obj.genes(5)); %Percentages of hunger and thirst respectively.
           obj.max_saturation = obj.saturation; %Establish the maximum saturation
           
           obj.destx = 0; %Used for mating AI
           obj.desty = 0; %Used for mating AI
           obj.dest_xy = [obj.destx obj.desty]; %Used for checks for mating AI
        ...
        function runAway(obj,predator_pos) 
             dir = obj.xy - predator_pos; %Establishes direction to go away from threat
             u_vec = [dir(1), dir(2)]; %placeholder vector
             obj.move_dir = u_vec/norm(u_vec); %Take the unit vector of the placeholder vector
        end
        %function finds the direction to runnaway from a predator
        
        function step(obj)
           % The main function for movement, status checks, state machine switches, etc.
           if ((obj.saturation <= 0) || (obj.lifespan == 0))
               obj.lifespan = 0; % Do this as a precaution to prevent errors
               obj.isDead = 1; % Doing this ensures the animal will be removed next step
           end
           ...
           switch obj.state_machine
               case "Roaming" %Check the state machine
                   obj.updatePosition;
           end
        ...
        function updatePosition(obj)
            if (isempty(obj.dest_xy) || ~any(obj.dest_xy)) %If the animal has no preset destination
                ...
                if ((obj.lifespan/obj.initial_lifespan) < 0.2) %If the animal is in the last 20% of its life
                    effective_speed = 0.8*effective_speed; %If animal is old, slow it down
                end
                obj.xy = obj.xy + (obj.move_dir * effective_speed); %Update position of object
                coordinates = obj.xy; %Save new coordinates to a variable
                obj.move_duration = obj.move_duration - 1; %Decrease move duration
                esize = obj.env_size; %Call environment size for boundary checks
                if (obj.move_duration == 0) %If it's time to change direction again...
                ...
                %checks if it is time and gives a animal a new direction
                %to wander in
                
                if (coordinates(1) < 0) %If x-value is negative
                    coordinates(1) = 0; %Move animal back inside the environment
                    obj.xy = coordinates;
                    uvec = [rand*0.2, (rand*2)-1]; %Deflect the animal away
                    obj.move_dir = uvec/norm(uvec);
                    obj.move_duration = obj.move_duration + randi([3, 5]);
                end
                %checks if an animal is outside the the environment, and if
                %they are, places them back inside it and directs them away
                %from the boundry
                
                if (coordinates(2) < 0) %If y-value is negative
                    coordinates(2) = 0; %Move animal back inside the environment
                    obj.xy = coordinates;
        ...
        function goto(obj, coordinate)
           %Makes the animal go towards something, will probably be used for hunger purposes
           selfcoord = obj.xy;
           direction_vec = coordinate - selfcoord;
           obj.move_dir = direction_vec/norm(direction_vec);
           obj.move_duration = randi([10, 15]);
        end
